...
- Image assets are referenced from Nift source using output-relative paths such as `@path('assets/images/...')`.
- Shared header, footer and documentation navigation must remain `@input` dependencies rather than copied markup.
- Internal page links must use tracked page names with `@path`.
